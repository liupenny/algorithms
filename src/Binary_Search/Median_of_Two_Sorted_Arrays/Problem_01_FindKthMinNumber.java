...
public class Problem_01_FindKthMinNumber {
	...
	public static int findKthNum(int[] arr1, int[] arr2, int kth) {
		...
		if (kth > l) {
			if (shorts[kth - l - 1] >= longs[l - 1]) {  //角标-1是因为k是第几个，对应数组都要-1
	...
	public static int getUpMedian(int[] a1, int s1, int e1, int[] a2, int s2, int e2) {  //迭代版求上中位数
		int mid1 = 0;
		int mid2 = 0;
		int offset = 0;
		while (s1 < e1) {
	...
	public static int[] getSortedAllArray(int[] arr1, int[] arr2) {   //将两个有序数组整合成一个数组并排序，去验证结果
		if (arr1 == null || arr2 == null) {
			throw new RuntimeException("Your arr is invalid!");
		}
		...
		Arrays.sort(arrAll);   //排序
		return arrAll;
	}

	public static int[] generateSortedArray(int len, int maxValue) {  //根据参数生成有序数组
		int[] res = new int[len];
		for (int i = 0; i != len; i++) {
