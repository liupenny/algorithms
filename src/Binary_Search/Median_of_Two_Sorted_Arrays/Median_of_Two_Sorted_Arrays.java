     ...
     */
    {
        ...
        if (k>l)
        {
            if(shorts[k-l-1] >= longs[l-1]) {
                return shorts[k - l - 1];      //这里两个比较应该包括 =
            } else if (longs[k-s-1] >= shorts[s-1]) {
                return longs[k - s - 1];   //角标-1是因为k是第几个，对应数组都要-1
            } else {
                return GetUpMedian(shorts, k - l, s - 1, longs, k - s, l - 1);  //因为上面k-l-1,k-s-1都测试过了不符合，所以从k-l,k-s开始
            }
        }
