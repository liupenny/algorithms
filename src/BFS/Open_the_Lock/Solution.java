package BFS.Open_the_Lock;

import java.util.*;

/**
 * Created by PennyLiu on 2018/9/7.
 */

public class Solution{
    public int openLock(String[] deadends, String target) {
        List<String> list = Arrays.asList(deadends);
        Set<String> dead = new HashSet<>(list);
        ...
        while (!queue.isEmpty()){
            ...
            ++turns;
            while (size-- > 0){
                String tmplock = queue.poll();
                for (int i = 0; i < 4; i++) {
                    for (int j = -1; j <= 1; j += 2) {
                        char[] chars = tmplock.toCharArray();
                        chars[i] = (char)(((chars[i] - '0') + j + 10) % 10 + '0'); //+10是为了得到9
